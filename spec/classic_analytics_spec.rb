...
describe Split::Analytics::ClassicAnalytics do
  ...
  it "should add custom variables for every test the user is involved in" do
    experiment1 = Split::Experiment.find_or_create("link_color", "blue", "red")
    experiment2 = Split::Experiment.find_or_create('link_text', 'Join', 'Signup')

    @ab_user = { experiment1.key => "blue", experiment2.key => "Join" }

    tracking_code = tracking_code(:account => 'UA-12345-6')
    expect(tracking_code).to eql("        <script type=\"text/javascript\">\n          var _gaq = _gaq || [];\n          _gaq.push(['_setAccount', 'UA-12345-6']);\n          \n          _gaq.push(['_setCustomVar', 1, 'link_color', 'blue', 1]);\n_gaq.push(['_setCustomVar', 2, 'link_text', 'Join', 1]);\n          _gaq.push(['_trackPageview']);\n          (function() {\n            var ga = document.createElement('script'); ga.type = 'text/javascript'; ga.async = true;\n            ga.src = ('https:' == document.location.protocol ? 'https://ssl.google-analytics.com/ga.js' : 'http://www.google-analytics.com/ga.js');\n            var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);\n          })();\n        </script>\n")
  end
